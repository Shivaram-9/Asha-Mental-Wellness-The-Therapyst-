...
app.post('/api/book/action', async (req, res) => {
    ...
    try {
        ...
        let displayHtml = `
            <div style="font-family: sans-serif; text-align: center; margin-top: 50px; max-width: 600px; margin-left: auto; margin-right: auto; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-radius: 8px; background: #fff;">
                <h2 style="color: ${color};">Booking ${msg}</h2>
                <div style="text-align: left; margin-top: 20px; padding: 15px; background: #f9f9f9; border-radius: 5px;">
                    ${customerEmailStatus}
                    ${calendarStatus}
                </div>
        `;
        ...
        if (!sideEffectsSuccess) {
            displayHtml += `
                <div style="margin-top: 20px; padding: 15px; background: #fff3f3; border: 1px solid #ffcdd2; border-radius: 5px;">
                    <p style="color: red; font-weight: bold;">Warning: Some operations failed.</p>
                    <p>The booking status was updated to ${action}, but calendar or email operations did not complete successfully.</p>
                    <p>You can refresh this page to retry the failed operations. (Idempotent retry is supported).</p>
                </div>
            `;
        } else {
            displayHtml += `
                <div style="margin-top: 20px;">
                    <p>All required operations succeeded. You can safely close this window.</p>
                </div>
            `;
        }
